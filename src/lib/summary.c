...
#include "summary.h"
#include <string.h>
static int ertf_resolve_control_word(FILE *);
// todo: deal with the case of multiple \info tags
/*
 * This function is called when \info tag is encountered.
 */
int ertf_summary(FILE *fp){

  int c;
  doc_info=(INFO *)malloc(sizeof(INFO));
  ...
  while((c=fgetc(fp))!=EOF){
    switch(c){
    case '{':
      if(ertf_resolve_control_word(fp))
	break;
      else
	goto error;
    case '}':return 1;
 ...
 */
static int ertf_resolve_control_word(FILE *fp){
  int c;
